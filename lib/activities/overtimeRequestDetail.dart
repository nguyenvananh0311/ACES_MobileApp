...
class GetOvertimeRequestScreen extends StatefulWidget {
  ...
  const GetOvertimeRequestScreen({
    super.key,
...
class _GetOvertimeRequestState extends State<GetOvertimeRequestScreen> {
  ...
  Future<bool> _checkLocationAndShowDialog(Branch branch) async {
    ...
    bool isCorrectLocation =
        await checkLocation(branch.latitude, branch.longitude, branch.distance);
    setState(() {
      isLoading = false;
    });
  ...
  Future<List<dynamic>> fetchData() async {
    return await Future.wait([
      ApiService()
          .fetchOTRequest(widget.requestId, companyId) // example employee ID
    ]);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: CustomAppBarScreen(
          title: localization!.overtimeRequestDetail,
          isBack: true,
        ),
                        ...
                        children: <Widget>[
                          ...
                          Row(
                            children: <Widget>[
                              ...
                              Text(
                                DateFormat('EEE dd MMMM y HH:mm')
                                    .format(otRequest!.fromDate),
                          ...
                          LeaveDetailItem(
                              label: "Total",
                              description: "${otRequest.duration} hour(s)"),
                          LeaveDetailItem(
                              label: "Phone number",
                              description: otRequest.contactNumber),
                          Row(
                            children: <Widget>[
                              ...
                              const Spacer(),
                              Text(
                                otRequest.status.toString(),
                                style: TextStyle(
                                    fontWeight: FontWeight.bold,
                                    color: otRequest.status == "Approved"
                                        ? Colors.green
                          ...
                          if (otRequest.updateByName != null)
                            LeaveDetailItem(
                                ...
                                description: otRequest.updateByName.toString()),
                          LeaveDetailItem(
                              label: localization.comment,
                              description: otRequest.comment != null
                                  ? otRequest.comment.toString()
                                  : ''),
                          LeaveDetailItem(
                              label: localization.reason,
                              description: otRequest.reason.toString()),
                          LeaveDetailItem(
                              label: localization.check_in,
                              description: otRequest.checkInTime == null
                                  ? ''
                                  : DateFormat('dd MMMM y hh:mm')
                                      .format(otRequest.checkInTime)),
                          LeaveDetailItem(
                              label: localization.check_out,
                              description: otRequest.checkOutTime == null
                                  ? ''
                                  : DateFormat('dd MMMM y hh:mm')
                                      .format(otRequest.checkOutTime)),
                          if (otRequest.fileName != null &&
                              otRequest.fileName != '')
                            Center(
                              child: CachedNetworkImage(
                                imageUrl:
                                    'https://203.176.128.5:5678/Images/Requests/${otRequest.fileName}',
                            ...
                            Center(
                              child: ElevatedButton(
                                onPressed: () async {
                                  ...
                                  if (response.isSuccess) {
                                    successToast(
                                        localization.canceledSuccessfully);
  ...
  bool checkTime(DateTime time, OvertimeRequest otRequest) {
    if (otRequest.fromDate.add(const Duration(minutes: -30)).isBefore(time) &&
        time.isBefore(otRequest.toDate.add(const Duration(hours: 1)))) {
      return true;
    }
    return false;
  }
